...
and make a PR. This bit requires a more in-depth explanation and where it really
diverges from the LFS Editor's Manual. If you are a BLFS editor, editing this
book will not be too different. Read [section 2](#2-locations) and
[section 3](#3-standards) for SLFS-specific information.

## 2. Locations
### 2.1. Internal files and directories
...
### 2.2. Book files and directories
...
### 3. Standards
Beyond knowing where files are, the contents of those files also counts, and
there are standards to keep in mind.

1. Edit `changelog.xml` as necessary. See [section 2.2.11](#2211-changelogxml).
...
[section 2.2.11](#2211-changelogxml)). Once you have made one, copy the text you
made, reformat that text you copied into plain text that won't get messed up in
a git commit message, then do a test render. See if the render looks fine, if it
rendered. Afterwards, do a `git status` to see you edited the correct files,
haven't made any new ones that shouldn't have been made, then `git add` those
files.
...
Now over on GitHub on your repo, make a pull request that merges into
`glfs-book:trunk`. If it has a conflict, repeat [section 4.2](#42-editing) to
fix those conflicts before making the PR. If all looks good, submit it.

An editor may request changes upon review. This is okay and routine. Repeat
[section 4.2](#42-editing) as necessary and GitHub will automatically pull in
the new changes to the PR. Rinse and repeat until the PR is merged.
